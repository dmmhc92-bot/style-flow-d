...
agent_communication:
    - agent: "main"
      message: "COMPLETE APP SYSTEM AUDIT. Test EVERY endpoint: AUTH (login, signup, forgot-password, me, delete-account with subscription cancellation), CLIENTS (full CRUD), FORMULAS (full CRUD), APPOINTMENTS (full CRUD), POSTS (full CRUD + delete own post), COMMENTS (CRUD + delete own comment), GALLERY (CRUD + delete), PROFILES (hub, avatar, portfolio delete), FEED (trending, following), ADMIN (guardian). Verify data isolation, delete cascades, and subscription cancellation on account delete. Credentials: admin@styleflow.com / Admin1234!"
    - agent: "testing"
